...
#Function
def GetDogSize(size): #filters the min_weight list to look for dog breeds based on size

    if size == "Tiny":
        for i in range(len(min_weight)):
            if min_weight[i] <= 10:
                filtered_list.append(dog_breed[i])
    ...
    recDog = input("Would you like a randomly selected dog?(Yes/No): ") #asks user if they want a randomly generated dog breed from the weight they are looking for

    if recDog == "Yes":

        recDog = random.choice(filtered_list)
        print(recDog)

def temperament_image():
    try:
        breed = input("Which dog would you like to view?: ") #search for dogs based of temperametn and displays their temperament and image
        position = dog_breed.index(breed)

        response = requests.get(url[position])
        img = Image.open(BytesIO(response.content))
        img.show()

        traits = temperament[position]
        print("Some temperaments for the " + breed + " are " + traits)

    except:
        print("Dog not found, please try again!")

def purpose(): #search for dog breeds based on the purpose of the dog and returns either the matching for "No matching breed"
    usage = input("What purpose do you need the dog for: ")
    usage = usage.lower()

    for i in range(len(dog_breed)):
        if usage in bred_for[i]:
            filtered_list.append(dog_breed[i])

    if len(filtered_list) == 0:
        return("No matching breeds")
...
def menu(): #main function combining (search for size, search for tempermant, search for purpose, and veiw all
    print("Welcome to dog breed generator")
    print("""Menu:
    1) search for size
    2) search for temperamant
    3) search for purpose
    4) veiw all
    5) quit""")
    while True:
        option = input("What would you like to do? (1-5): ") #user input for which option they would like to perform

        if option == "1":
            size = input("What size dog breed would you like to search for? (Tiny, Small, Medium, or Large): ")
        ...
        if option == "5":
            print("Thank you for using dog breed generator!")
            break
